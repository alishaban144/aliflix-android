...
classes=com.aliflix.app.player.NativeBackgroundPlaybackTest,com.aliflix.app.player.WebStreamHandoffTest,com.aliflix.app.player.NativeSkipControlsTest,com.aliflix.app.DetailsStabilityTest,com.aliflix.app.data.ExactProgressTest,com.aliflix.app.player.CastSubtitleTracksTest
if [ "$API" = 37.0 ]; then
  ...
  # am instrument can exit zero even when assertions fail. Require all seven tests to pass.
  tr -d '\r' < .validation/instrumentation.txt | grep -Fxq 'OK (7 tests)'
else

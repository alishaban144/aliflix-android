...
/**
 * Deterministically ranks catalogue search results without changing their source set.
 *
 * Title relevance is deliberately worth far more than popularity. A rating can refine
 * otherwise equivalent matches, but it cannot make a popular partial match beat an exact one.
 */
object SearchRanker {
    ...
    private fun parseQuery(rawQuery: String): ParsedQuery {
        ...
        // Intent qualifiers are consumed only from the end. This avoids interpreting
        // title words in queries such as "Movie 43" or "2001 A Space Odyssey".

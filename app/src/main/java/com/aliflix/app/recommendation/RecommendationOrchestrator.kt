...
class CatalogRecommendationCandidateRepository(
    ...
    suspend fun resolveAiCandidates(
        ...
        limit: Int = 350,
        ledger: RetrievalLedger = RetrievalLedger(),
        additionalKeywordPhrases: List<String> = emptyList(),
        additionalPairwiseSearches: List<PairwiseSearch> = emptyList(),
        additionalBroadPhrases: List<String> = emptyList(),
    ): AiCandidateResolution = supervisorScope {
        val gate = Semaphore(4)
        val mediaType = if (interpretation.hardConstraints.mediaType.equals("tv", ignoreCase = true)) MediaType.TV else MediaType.MOVIE

        // 1. Resolve keyword phrases by concept group
        val conceptGroups = interpretation.requiredConceptGroups
        val keywordGroups = mutableListOf<ResolvedKeywordGroup>()
        var totalKeywordIdsCount = 0

        conceptGroups.forEach { group ->
            val phrases = (listOf(group.label) + group.alternatives + group.specificSubtypes +
                interpretation.keywordSearchPhrases.filter { phrase ->
                    phrase.contains(group.label, ignoreCase = true) ||
                        group.alternatives.any { phrase.contains(it, ignoreCase = true) }
                }).distinct()

            val keywordObjs = phrases.map { phrase ->
                async {
                    if (!ledger.searchedKeywordPhrases.add(phrase.lowercase())) return@async emptyList<ResolvedKeyword>()
                    gate.withPermit {
                        try { client.scrapeTmdbKeywordSearch(phrase) } catch (_: Exception) { emptyList() }
                    }
                }
            }.awaitAll().flatten().distinctBy { it.id }

            val ids = keywordObjs.map { it.id }
            ledger.resolvedKeywordIds.addAll(ids)
            totalKeywordIdsCount += ids.size
            keywordGroups.add(ResolvedKeywordGroup(groupId = group.id, keywordIds = ids, phrases = phrases))
        }

        // Additional standalone keyword phrases
        val extraPhrases = (interpretation.keywordSearchPhrases + additionalKeywordPhrases)
            .filter { phrase -> ledger.searchedKeywordPhrases.add(phrase.lowercase()) }
        if (extraPhrases.isNotEmpty()) {
            val extraKwObjs = extraPhrases.map { phrase ->
                async {
                    gate.withPermit {
                        try { client.scrapeTmdbKeywordSearch(phrase) } catch (_: Exception) { emptyList() }
                    }
            ...
            val ids = extraKwObjs.map { it.id }
            ledger.resolvedKeywordIds.addAll(ids)
            totalKeywordIdsCount += ids.size
            if (ids.isNotEmpty()) {
                keywordGroups.add(ResolvedKeywordGroup(groupId = "extra", keywordIds = ids, phrases = extraPhrases))
            }
        }

        com.aliflix.app.data.omdb.OmdbDiagnostics.requiredConceptGroupCount.set(conceptGroups.size)
        com.aliflix.app.data.omdb.OmdbDiagnostics.interpretationKeywordPhrases.set(interpretation.keywordSearchPhrases.size)
        com.aliflix.app.data.omdb.OmdbDiagnostics.resolvedKeywordIdsCount.set(totalKeywordIdsCount)

        // 2. Discover TMDB pages across multiple deep search paths
        val rawDiscovered = mutableListOf<Media>()
        val seenKeys = mutableSetOf<String>()

        fun addMediaList(list: List<Media>) {
            for (item in list) {
                if (seenKeys.add(item.key) && ledger.discoveredTmdbIds.add(item.id)) {
                    rawDiscovered.add(item)
                }
            }
        }

        val genreIdsStr = interpretation.hardConstraints.includedGenres.mapNotNull { genreName: String ->
            com.aliflix.app.data.GenreCatalog.specFor(genreName, mediaType)?.genreIds?.firstOrNull()
        }.joinToString(",")

        // 2a. Broad title search phrases
        val allBroadPhrases = (interpretation.broadSearchPhrases + additionalBroadPhrases).distinct()
        for (phrase in allBroadPhrases) {
            if (rawDiscovered.size >= limit) break
            if (!ledger.searchedBroadPhrases.add(phrase.lowercase())) continue
            try {
                val results = gate.withPermit { client.search(phrase) }
                addMediaList(results.filter { it.type == mediaType })
            } catch (_: Exception) {}
        }
        ...
        // Path A: Hard genre discover pages (pages 1 to 5)
        if (genreIdsStr.isNotBlank()) {
            paths.add("with_genres=$genreIdsStr" to 5)
        }

        // Path B: Individual concept group keyword pages (pages 1 to 4)
        keywordGroups.forEach { group ->
            if (group.keywordIds.isNotEmpty()) {
                val kwStr = group.keywordIds.take(10).joinToString("|")
                val params = if (genreIdsStr.isNotBlank()) "with_genres=$genreIdsStr&with_keywords=$kwStr" else "with_keywords=$kwStr"
                paths.add(params to 4)
            }
        }

        // Path C: Pairwise concept group combinations (pages 1 to 3)
        if (keywordGroups.size >= 2) {
            for (i in 0 until keywordGroups.size) {
                for (j in i + 1 until keywordGroups.size) {
                    val g1 = keywordGroups[i].keywordIds
                    val g2 = keywordGroups[j].keywordIds
                    if (g1.isNotEmpty() && g2.isNotEmpty()) {
                        val kw1 = g1.take(5).joinToString("|")
                        val kw2 = g2.take(5).joinToString("|")
                        val params = "with_keywords=$kw1&$kw2"
                        paths.add(params to 3)
                    }
                }
            }
        }

        // Path D: Additional expansion pairwise searches
        for (pw in additionalPairwiseSearches) {
            val kwLeft = gate.withPermit { client.scrapeTmdbKeywordSearch(pw.leftConcept) }.map { it.id }
            val kwRight = gate.withPermit { client.scrapeTmdbKeywordSearch(pw.rightConcept) }.map { it.id }
            if (kwLeft.isNotEmpty() && kwRight.isNotEmpty()) {
                val params = "with_keywords=${kwLeft.take(4).joinToString("|")}&${kwRight.take(4).joinToString("|")}"
                paths.add(params to 3)
            }
        }

        // Execute paths across pages
        for ((params, maxPages) in paths) {
            if (rawDiscovered.size >= limit) break
            if (!ledger.searchedDiscoverParams.add(params)) continue
            com.aliflix.app.data.omdb.OmdbDiagnostics.searchPathCount.incrementAndGet()

            for (page in 1..maxPages) {
                if (rawDiscovered.size >= limit) break
                val pageKey = "$params&page=$page"
                if (!ledger.searchedPages.add(pageKey)) continue
                com.aliflix.app.data.omdb.OmdbDiagnostics.pagesSearched.incrementAndGet()

                try {
                    val pageResults = gate.withPermit { client.scrapeTmdbDiscoverPage(params, mediaType, page) }
                    if (pageResults.isEmpty()) break
                    val countBefore = rawDiscovered.size
                    addMediaList(pageResults)
                    if (rawDiscovered.size == countBefore) break
                } catch (_: Exception) {
                    break
                }
            }
        ...
        com.aliflix.app.data.omdb.OmdbDiagnostics.rawCandidates.set(rawDiscovered.size)

        // 3. Local enrichment (verifyRecommendationItem)
        val verifiedPairs = rawDiscovered.take(limit).map { media ->
            async {
                gate.withPermit {
                    try {
                        val required = RequiredMetadataFields(
    ...
    suspend fun resolveSimilarToCandidates(
        anchor: RecommendationCandidate,
        mediaKind: RecommendationMediaKind,
        spec: CatalogDiscoverySpec,
        limit: Int = 350,
        ...
        val gate = Semaphore(4)
        val mediaType = mediaKind.mediaType
        val enrichedAnchor = enrichSimilarityAnchor(anchor)
        val rawDiscovered = mutableListOf<Media>()
        val seenKeys = mutableSetOf<String>()

        fun addMediaList(list: List<Media>) {
            for (item in list) {
                if (item.key != enrichedAnchor.media.key && seenKeys.add(item.key)) {
                    rawDiscovered.add(item)
                }
            }
        }

        // 1. Direct provider related titles (Highest priority)
        val directRelated = try {
            client.relatedRecommendationItems(enrichedAnchor.media)
                .filter { it.media.type == mediaType }
                .map { it.media }
        } catch (_: Exception) { emptyList() }
        addMediaList(directRelated)

        // 2. Search anchor title for exact catalogue & TMDB matches
        try {
            val titleSearch = client.search(enrichedAnchor.media.title).filter { it.type == mediaType }
            addMediaList(titleSearch)
        } catch (_: Exception) {}

        val anchorKeywords = try {
            client.scrapeTmdbKeywordSearch(enrichedAnchor.media.title)
        } catch (_: Exception) { emptyList() }

        // 3. TMDB Discover using anchor genres & keywords
        val genreIdsStr = enrichedAnchor.media.genres.mapNotNull { genreName ->
            com.aliflix.app.data.GenreCatalog.specFor(genreName, mediaType)?.genreIds?.firstOrNull()
        }.joinToString(",")
        ...
        if (genreIdsStr.isNotBlank()) {
            for (page in 1..4) {
                if (rawDiscovered.size >= limit) break
                try {
                    val pageRes = gate.withPermit { client.scrapeTmdbDiscoverPage("with_genres=$genreIdsStr", mediaType, page) }
                    if (pageRes.isEmpty()) break
                    addMediaList(pageRes)
                } catch (_: Exception) { break }
            }
        }

        if (anchorKeywords.isNotEmpty()) {
            val kwStr = anchorKeywords.take(6).map { it.id }.joinToString("|")
            val params = if (genreIdsStr.isNotBlank()) "with_genres=$genreIdsStr&with_keywords=$kwStr" else "with_keywords=$kwStr"
            for (page in 1..4) {
                if (rawDiscovered.size >= limit) break
                try {
                    val pageRes = gate.withPermit { client.scrapeTmdbDiscoverPage(params, mediaType, page) }
                    if (pageRes.isEmpty()) break
                    addMediaList(pageRes)
                } catch (_: Exception) { break }
            }
        ...
        val verifiedPairs = rawDiscovered.take(limit).map { media ->
            async {
                gate.withPermit {
                    try {
                        val required = RequiredMetadataFields(
                            runtime = mediaType == MediaType.MOVIE,
                            tvEpisodeRuntime = mediaType == MediaType.TV,
                            rottenTomatoesRating = true,
                            imdbRating = true,
                        )
                        val verified = client.verifyRecommendationItem(media, required)

...
`NativeBackgroundPlaybackTest` verifies actual changing decoded pixels, Settings background playback, phone power events, notification pause/play, Activity recreation/destruction, display restoration and Stop casting. Phone launches explicitly target display 0, including notification launches, so the last focused TV cannot absorb the controls. A shared-display fixture tests recovery when Android sleeps the shared power group; API 35 may still report its virtual Display as ON while its windows sleep. An independent-display fixture uses AOSP system display flags solely during fixture creation. Video-while-locked assertions apply to the independent fixture while its display stays powered. Shared/asleep routes are explicitly logged as not proving phone-lock video. Playback and restoration assertions remain mandatory; playback runs with the normal app UID. This distinction must not be interpreted as proof of an OEM wireless route's power policy. `WebStreamHandoffTest` verifies real WebView video/HLS extraction. `NativeSkipControlsTest` verifies timed native buttons, seeking, post-credit preservation and control lock. Unit tests cover malformed/stale IntroDB data and provider ordering/routes.

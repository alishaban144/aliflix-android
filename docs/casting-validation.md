...
The release workflow requires these phone-emulator tests on Android 15 before publishing, together with Android unit tests, lint, Worker checks, and signed-APK integrity checks. Both tests also passed locally on Android 17 (API 37); the recorded instrumentation output is in `test-results/android17-casting.txt`. Android 17 test setup grants the actual local-network permission; it does not disable the platform's LAN restrictions. Linux CI uses software decoding because the emulator's Goldfish hardware decoder loses color buffers on the virtual presentation display. API 37 Linux CI is excluded because its mapper.ranchu repeatedly aborts SurfaceFlinger with `hasReadColorBufferDma`, restarting Android before app installation; command-line and environment feature overrides did not resolve that upstream incompatibility. The same real decoded-frame assertions remain required on Android 15 CI and were exercised by the local Android 17 run.
